...
public class ValidateabChecksB implements Adaptee{
    private List<String> validationStatus;

    public ValidateabChecksB(ClassAB classAB){
        validationStatus = Commons.checkInteger(classAB.getField2());
    }
    ...
    public Map<String, List<String>> speciifcReason() {
        ...
        if(validationStatus.size() >0){
            reasonMap.put(Constents.CLASS_B_OF_AB, validationStatus);
        }
        return reasonMap;
    }
}
